...
funcCallExpr    : ID LPAREN argList RPAREN
                 {
                    ...
                    if (!ST_lookup("output")){
                        int outIndex = ST_insert("output","global",2,2,0);
                        symEntry* outEntry = ST_lookup("output");
                        add_param(outEntry, 0, 0);
                    }
                    symEntry* tempEntry = ST_lookup($1);
                    if (tempEntry == NULL){
...
void getTypesFunc(tree* tempTree){
   if (tempTree->nodeKind == IDENTIFIER){
      ...
      if (tempTree->nodeKind == INTEGER){
         param *new_param = (param *)malloc(sizeof(param));
         new_param->data_type = 0;
         new_param->symbol_type = 0;
         new_param->next = NULL;
